...
window.ORAL = { SCHWERPUNKTE: [
...
], QUESTIONS: [
  {
    ...
    "musterantwort": "Ein $b$-adisches Stellenwertsystem ($b$ eine natürliche Zahl, $b > 1$) stellt jede natürliche Zahl eindeutig durch Bündelung in $b$-er-Potenzen dar: $n = a_m \\cdot b^m + a_{m-1} \\cdot b^{m-1} + \\dots + a_1 \\cdot b^1 + a_0 \\cdot b^0$. Der Wert einer Ziffer hängt von ihrer Stelle (ihrem Stellenwert $b^k$) ab. Für die Ziffern gilt $0 \\le a_k < b$, d. h. es gibt genau $b$ verschiedene Ziffern ($0, 1, \\dots, b-1$); das Zehnersystem ist der Sonderfall $b = 10$.",
    "kriterien": [
      "Nennt die Summendarstellung mit Potenzen der Basis $b$ (Polynomform)",
      "Gibt die Ziffernbedingung $0 \\le a_k < b$ korrekt an",
  ...
  },
  {
    ...
    "musterantwort": "Man dividiert fortlaufend durch 2 und notiert die Reste: $34 = 17 \\cdot 2 + 0$, $17 = 8 \\cdot 2 + 1$, $8 = 4 \\cdot 2 + 0$, $4 = 2 \\cdot 2 + 0$, $2 = 1 \\cdot 2 + 0$, $1 = 0 \\cdot 2 + 1$. Die Reste werden von unten nach oben gelesen: $34 = (100010)_2$. Zur Kontrolle: $(100010)_2 = 2^5 + 2^1 = 32 + 2 = 34$. Alternativ über die größte Zweierpotenz: $34 = 32 + 2 = 2^5 + 2^1$.",
    "kriterien": [
  ...
  },
  {
    ...
    "schwerpunkt": "stellenwert",
    "stufe": "beweis",
    "frage": "Begründen Sie, warum im $b$-adischen Stellenwertsystem jede Ziffer die Bedingung $0 \\le a_k < b$ erfüllen muss und warum die Darstellung dann eindeutig ist.",
    "musterantwort": "Wäre eine Ziffer $a_k \\ge b$, so ließe sich $b \\cdot b^k = b^{k+1}$ abspalten, d. h. man könnte ein weiteres Bündel der nächsthöheren Stufe bilden — die Darstellung wäre nicht in reduzierter Form. Die Bedingung $0 \\le a_k < b$ sichert also, dass an jeder Stelle nur der 'nicht weiter bündelbare' Rest steht. Die Eindeutigkeit folgt aus dem Divisionsalgorithmus: Bei fortgesetzter Division durch $b$ ist in $n = q \\cdot b + a_0$ der Rest $a_0$ mit $0 \\le a_0 < b$ eindeutig bestimmt; iteriert man mit $q$, ergeben sich alle weiteren Ziffern eindeutig. Also besitzt jede natürliche Zahl genau eine $b$-adische Darstellung.",
    "kriterien": [
      "Verknüpft die Ziffernschranke mit dem Bündelungsgedanken ($a_k \\ge b \\Rightarrow$ Übertrag zur nächsten Potenz)",
  ...
  },
  {
    ...
    "musterantwort": "Eine natürliche Zahl $a$ teilt eine natürliche Zahl $b$ (geschrieben $a \\mid b$) genau dann, wenn es eine natürliche Zahl $c$ gibt mit $a \\cdot c = b$. Gilt $a \\mid b$, so heißt $a$ ein Teiler von $b$ (und $b$ ein Vielfaches von $a$). Beispiel: $2 \\mid 6$, denn mit $c = 3$ gilt $2 \\cdot 3 = 6$. Entscheidend ist die Existenz eines solchen $c$ innerhalb der natürlichen Zahlen; die Teilbarkeit ist also über die Multiplikation und nicht über das Rest-Rechnen definiert.",
    "kriterien": [
      "Nennt die Existenzbedingung: es gibt ein $c \\in \\mathbb{N}$ mit $a \\cdot c = b$",
      "Verwendet die korrekte Fachsprache/Notation ($a \\mid b$, Teiler, Vielfaches)",
  ...
  },
  {
    ...
    "musterantwort": "Die Primfaktorzerlegung ist $12 = 2^2 \\cdot 3$. Alle Teiler entstehen, indem man jeden Primfaktor mit einem Exponenten von 0 bis zum Maximum kombiniert: $2^0 \\cdot 3^0 = 1$, $2^1 = 2$, $2^2 = 4$, $3$, $2 \\cdot 3 = 6$, $4 \\cdot 3 = 12$. Damit ist die Teilermenge $T(12) = \\{1, 2, 3, 4, 6, 12\\}$. Man kann die Teiler auch paarweise suchen ($1 \\cdot 12$, $2 \\cdot 6$, $3 \\cdot 4$) und so sicherstellen, dass keiner fehlt. Systematisch liefert die Primfaktorzerlegung genau alle Teiler.",
    "kriterien": [
      "Gibt die Primfaktorzerlegung $12 = 2^2 \\cdot 3$ korrekt an",
  ...
  },
  {
    ...
    "schwerpunkt": "teilbarkeit",
    "stufe": "beweis",
    "frage": "Beweisen Sie die Summenregel der Teilbarkeit: Wenn $a \\mid b$ und $a \\mid c$ gilt, dann gilt auch $a \\mid (b + c)$.",
    "musterantwort": "Voraussetzung mit der Definition umgesetzt: Aus $a \\mid b$ folgt es gibt $p \\in \\mathbb{N}$ mit $a \\cdot p = b$, aus $a \\mid c$ folgt es gibt $q \\in \\mathbb{N}$ mit $a \\cdot q = c$. Dann gilt $b + c = a \\cdot p + a \\cdot q = a \\cdot (p + q)$. Da $p + q$ wieder eine natürliche Zahl ist, existiert also ein Faktor, mit dem $a$ multipliziert $(b + c)$ ergibt. Nach Definition der Teilbarkeit folgt $a \\mid (b + c)$. Die tragende Idee ist das Ausklammern des gemeinsamen Teilers $a$ mittels des Distributivgesetzes.",
    "kriterien": [
      "Setzt beide Voraussetzungen korrekt als $a \\cdot p = b$ und $a \\cdot q = c$ an",
      "Klammert $a$ aus: $b + c = a \\cdot (p + q)$ (Distributivgesetz)",
      "Schließt regelkonform mit der Definition auf $a \\mid (b + c)$ zurück"
    ],
    "quelle": "MAL1-1 – Teilbarkeit, Teilermengen & Teilbarkeitsregeln",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Teilbarkeit, Teilermengen & Teilbarkeitsregeln"
  },
  {
    ...
    "schwerpunkt": "teilbarkeit",
    "stufe": "vertiefung",
    "frage": "Die Teilbarkeitsrelation ist transitiv, und es gibt neben der Summenregel auch eine „andere Summenregel\" (aus $a \\mid b$ und $a \\mid (b + c)$ folgt $a \\mid c$). Wie hängen diese Aussagen zusammen, und wo liegen die Grenzen – gilt etwa die Umkehrung der Summenregel?",
    "musterantwort": "Transitivität (aus $a \\mid b$ und $b \\mid c$ folgt $a \\mid c$) und die Summenregeln lassen sich alle direkt aus der Definition über Ausklammern beweisen; die „andere Summenregel\" ergibt sich, indem man $c = (b + c) - b = a \\cdot q - a \\cdot p = a \\cdot (q - p)$ schreibt und so aus zwei Teilbarkeiten auf eine dritte schließt – sie ist also die „Differenz-Variante\" der Summenregel. Grenzen: Die Umkehrung der Summenregel gilt nicht. Aus $a \\mid (b + c)$ folgt nicht $a \\mid b$ und $a \\mid c$; Gegenbeispiel: $3 \\mid (2 + 4) = 6$, aber $3 \\nmid 2$ und $3 \\nmid 4$. Auch bei der Differenz muss man im Bereich der natürlichen Zahlen $q \\ge p$ sichern, damit $q - p \\in \\mathbb{N}$ bleibt.",
    "kriterien": [
      "Erklärt, dass alle Regeln aus derselben Beweisidee (Definition + Ausklammern) folgen",
      "Leitet die andere Summenregel über die Differenz $c = a \\cdot (q - p)$ her",
      "Widerlegt die Umkehrung mit einem Gegenbeispiel und benennt Grenzfälle (z. B. $q \\ge p$ in $\\mathbb{N}$)"
    ],
    "quelle": "MAL1-1 – Teilbarkeit, Teilermengen & Teilbarkeitsregeln",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Teilbarkeit, Teilermengen & Teilbarkeitsregeln"
  ...
  },
  {
    ...
    "musterantwort": "Man zerlegt 12 schrittweise in Faktoren: $12 = 2 \\cdot 6$, dann $6 = 2 \\cdot 3$. Die letzten „Blätter“ des Baums sind Primzahlen, weiter zerlegen kann man nicht. Somit ist $12 = 2 \\cdot 2 \\cdot 3 = 2^2 \\cdot 3$. Unabhängig davon, ob man mit $2 \\cdot 6$, $3 \\cdot 4$ oder $4 \\cdot 3$ beginnt, erhält man stets dieselben Primfaktoren mit denselben Vielfachheiten.",
    "kriterien": [
      "Zerlegt systematisch bis nur noch Primzahlen als Faktoren auftreten",
      "Ergebnis korrekt: $12 = 2^2 \\cdot 3$, inklusive Potenzschreibweise",
  ...
  },
  {
    ...
    "musterantwort": "Widerspruchsannahme: Es gebe nur endlich viele Primzahlen $p_1, p_2, \\dots, p_n$. Man bildet die Zahl $a = p_1 \\cdot p_2 \\cdot \\dots \\cdot p_n + 1$. Diese Zahl $a$ ist größer als 1 und besitzt daher einen Primteiler $p$. Dieser $p$ muss unter den $p_1, \\dots, p_n$ vorkommen (nach Annahme sind das alle). Dann teilt $p$ aber sowohl das Produkt $p_1 \\cdot \\dots \\cdot p_n$ als auch $a$, also nach der Summenregel auch die Differenz $a - (p_1 \\cdot \\dots \\cdot p_n) = 1$. Kein Primteiler teilt jedoch 1 – Widerspruch. Also war die Annahme falsch und die Menge der Primzahlen ist unendlich.",
    "kriterien": [
  ...
  },
  {
    ...
    "musterantwort": "Der Fundamentalsatz der Arithmetik besagt: Jede natürliche Zahl $n > 1$ lässt sich als Produkt von Primzahlen darstellen, und diese Darstellung ist – bis auf die Reihenfolge der Faktoren – eindeutig. Die Existenz sichert, dass jede Zahl überhaupt zerlegbar ist; die Eindeutigkeit garantiert, dass die Primfaktoren mit ihren Vielfachheiten eindeutig bestimmt sind (z. B. $12 = 2^2 \\cdot 3$ und keine andere Kombination). Würde man die 1 als Primzahl zulassen, könnte man beliebig Faktoren 1 ergänzen ($12 = 2^2 \\cdot 3 = 1 \\cdot 2^2 \\cdot 3 = 1^2 \\cdot 2^2 \\cdot 3 \\dots$) und die Eindeutigkeit ginge verloren. Genau deshalb wird die 1 ausgeschlossen.",
    "kriterien": [
  ...
  },
  {
    ...
    "musterantwort": "Zwei ganze Zahlen $a$ und $b$ heissen kongruent modulo $m$ (geschrieben $a \\equiv b \\pmod{m}$), wenn sie beim Teilen durch $m$ denselben Rest lassen, gleichbedeutend: wenn $m$ die Differenz $a - b$ teilt. Die Restklasse $\\overline{r}$ modulo $m$ ist die Menge aller ganzen Zahlen mit demselben Rest $r$, also $\\overline{r} = \\{n \\in \\mathbb{Z} \\mid n = m \\cdot k + r,\\ k \\in \\mathbb{Z}\\}$. Fuer $m = 3$ gibt es genau drei Restklassen: $\\overline{0} = \\{\\dots, -6, -3, 0, 3, 6, \\dots\\}$ ($n = 3k$), $\\overline{1} = \\{\\dots, -2, 1, 4, 7, \\dots\\}$ ($n = 3k+1$) und $\\overline{2} = \\{\\dots, -1, 2, 5, 8, \\dots\\}$ ($n = 3k+2$).",
    "kriterien": [
      ...
      "Beispiel $m = 3$ mit den drei Restklassen $\\overline{0}$, $\\overline{1}$, $\\overline{2}$ (Form $3k+r$)"
    ],
  ...
  },
  {
    ...
    "musterantwort": "Man nutzt fortgesetzte Division mit Rest (Satz von der eindeutigen Division mit Rest, $n = q \\cdot m + r$, $0 \\le r < m$): $372 = 4 \\cdot 84 + 36$; $84 = 2 \\cdot 36 + 12$; $36 = 3 \\cdot 12 + 0$. Der letzte von Null verschiedene Rest ist 12, also $\\operatorname{ggT}(372, 84) = 12$. Tragend ist, dass $\\operatorname{ggT}(n, m) = \\operatorname{ggT}(m, r)$ gilt, weil jeder gemeinsame Teiler von $n$ und $m$ auch $r = n - q \\cdot m$ teilt und umgekehrt; der Algorithmus bricht ab, weil die Reste streng fallen.",
    "kriterien": [
      "Divisionskette korrekt durchgeführt bis Rest 0 (Ergebnis 12)",
      "Letzter nichtverschwindender Rest als ggT identifiziert",
      "Begründung über Division mit Rest und Invarianz $\\operatorname{ggT}(n,m) = \\operatorname{ggT}(m,r)$"
    ],
    "quelle": "MAL1-1 – Kongruenzen, Restklassen & ggT/Euklid",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Kongruenzen, Restklassen & ggT/Euklid"
  },
  {
    ...
    "schwerpunkt": "kongruenzen",
    "stufe": "beweis",
    "frage": "Beweisen Sie, dass in den Restklassen modulo 3 gilt: $\\overline{2} + \\overline{2} = \\overline{1}$. Warum ist dieses Ergebnis unabhängig von der Wahl der Repräsentanten?",
    "musterantwort": "Seien $a \\in \\overline{2}$ und $b \\in \\overline{2}$, also $a = 3k_1 + 2$ und $b = 3k_2 + 2$ mit $k_1, k_2 \\in \\mathbb{N}_0$. Dann ist $a + b = 3k_1 + 3k_2 + 4 = 3(k_1 + k_2 + 1) + 1$, also von der Form $3k + 1$ und damit in $\\overline{1}$. Die Rechnung hängt nicht von den konkreten $k_1, k_2$ ab, sondern nur von den Resten 2 und 2; deshalb liefert jede Wahl von Repräsentanten dasselbe Ergebnis $\\overline{1}$ – die Addition der Restklassen ist wohldefiniert.",
    "kriterien": [
      "Allgemeine Repräsentanten $a = 3k_1+2$, $b = 3k_2+2$ angesetzt",
      "Umformung zu $3(k_1+k_2+1)+1$ und Zuordnung zu $\\overline{1}$",
  ...
  },
  {
    ...
    "musterantwort": "Der Satz von der eindeutigen Division mit Rest (zu $n, m$ gibt es genau ein Paar $q, r$ mit $n = q \\cdot m + r$, $0 \\le r < m$) garantiert, dass jede Zahl in genau eine der $m$ Restklassen fällt – die Eindeutigkeit des Restes macht die Einteilung in Restklassen überhaupt widerspruchsfrei. Der Grenzfall $r = 0$ ist gerade die Teilbarkeit ($n = q \\cdot m$), also die Restklasse $\\overline{0}$. Lässt man die Bedingung $0 \\le r < m$ fallen und erlaubt negative Reste, so kann man dieselbe Zahl anders darstellen, z. B. $11 = 3 \\cdot 3 + 2$ (positiver Rest) oder $11 = 4 \\cdot 3 - 1$ (negativer Rest, „Schulden beim Teilen“); die Eindeutigkeit geht dann verloren, weshalb die Einschränkung $0 \\le r < m$ nötig ist.",
    "kriterien": [
      "Eindeutigkeit des Restes als Grundlage der Restklasseneinteilung erklärt",
      "Grenzfall $r = 0$ als Teilbarkeit / Restklasse $\\overline{0}$ erkannt",
      "Negative Reste am Beispiel ($11 = 4 \\cdot 3 - 1$) und Verlust der Eindeutigkeit ohne $0 \\le r < m$"
    ],
    "quelle": "MAL1-1 – Kongruenzen, Restklassen & ggT/Euklid",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Kongruenzen, Restklassen & ggT/Euklid"
  },
  {
    ...
    "kriterien": [
      ...
      "Mindestens eine typische Fehlvorstellung benannt (z. B. Rest $\\ge$ Divisor, Rest 0)"
    ],
    "quelle": "MAL1-1 – Kongruenzen, Restklassen & ggT/Euklid",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Kongruenzen, Restklassen & ggT/Euklid"
  ...
  },
  {
    ...
    "schwerpunkt": "logik_beweis",
    "stufe": "verfahren",
    "frage": "Erstellen Sie die Wahrheitstafel der Verknüpfung $A \\Rightarrow B$ und bilden Sie deren Verneinung. Was ist das Gegenteil von „Wenn die Sonne scheint, dann gehe ich joggen“?",
    "musterantwort": "$A \\Rightarrow B$ ist definiert als $\\neg A \\lor B$ und in den vier Zeilen (ww, wf, fw, ff) genau in der Zeile wf ($A$ wahr, $B$ falsch) falsch, sonst wahr. Die Verneinung $\\neg(A \\Rightarrow B)$ ist folglich $A \\land \\neg B$ und nur in der Zeile wf wahr. Sprachlich: Das Gegenteil ist „Die Sonne scheint, und ich gehe (trotzdem) nicht joggen.“",
    "kriterien": [
      "vollständige Wahrheitstafel mit allen vier Belegungen",
      "$A \\Rightarrow B$ nur bei $A \\land \\neg B$ falsch erkannt",
      "Verneinung $\\neg(A \\Rightarrow B) = A \\land \\neg B$ korrekt gebildet und versprachlicht"
    ],
    "quelle": "MAL1-1 – Aussagenlogik & Beweisformen",
    "gruppe": "Logik & Beweise",
    "schwerpunkt_name": "Aussagenlogik & Beweisformen"
  },
  {
    ...
    "musterantwort": "Widerspruchsbeweis: Man nimmt an, die zu zeigende Aussage sei falsch, und leitet daraus einen Widerspruch her. Annahme: Es gibt eine größte natürliche Zahl $n$. Dann ist aber $n+1$ ebenfalls eine natürliche Zahl und $n+1 > n$, im Widerspruch zur Maximalität von $n$. Also ist die Annahme falsch und es gibt keine größte natürliche Zahl. Beim direkten Beweis schließt man dagegen ohne Gegenannahme von den Voraussetzungen über gültige Implikationen ($A \\Rightarrow B$) direkt auf die Behauptung.",
    "kriterien": [
  ...
  },
  {
    ...
    "schwerpunkt": "logik_beweis",
    "stufe": "vertiefung",
    "frage": "Erläutern Sie das Prinzip der vollständigen Induktion und wie es mit dem Aufbau der natürlichen Zahlen zusammenhängt. Beweisen Sie exemplarisch, dass $1+2+\\dots+n = \\frac{n(n+1)}{2}$ gilt.",
    "musterantwort": "Die vollständige Induktion nutzt den induktiven Aufbau von $\\mathbb{N}$ (jede Zahl durch fortgesetztes $+1$ erreichbar): Aus Induktionsanfang (Aussage gilt für $n=1$) und Induktionsschritt (gilt sie für $n$, so auch für $n+1$) folgt die Gültigkeit für alle $n$. Anfang: $n=1$ liefert $1 = \\frac{1 \\cdot 2}{2} = 1$. Schritt: Gelte $1+\\dots+n = \\frac{n(n+1)}{2}$ (IV). Dann $1+\\dots+n+(n+1) = \\frac{n(n+1)}{2} + (n+1) = \\frac{(n+1)(n+2)}{2}$, also die Formel für $n+1$. Damit gilt sie für alle $n \\in \\mathbb{N}$.",
    "kriterien": [
      "Induktionsanfang und Induktionsschritt klar getrennt",
      "Induktionsvoraussetzung im Schritt korrekt verwendet",
      "Zusammenhang zum Nachfolgeraufbau von $\\mathbb{N}$ benannt"
    ],
    "quelle": "MAL1-1 – Aussagenlogik & Beweisformen",
    "gruppe": "Logik & Beweise",
    "schwerpunkt_name": "Aussagenlogik & Beweisformen"
  ...
  },
  {
    ...
    "musterantwort": "Eine (unendliche) Zahlfolge ist eine Funktion, die jeder natürlichen Zahl $n$ (dem Index) genau ein Folgenglied $a_n$ aus einer Zielmenge $X$ (bei uns den natürlichen bzw. reellen Zahlen) zuordnet; entscheidend ist die Eindeutigkeit (genau ein Wert je Index). Bei einer endlichen Folge ist der Definitionsbereich eine endliche Teilmenge von $\\mathbb{N}$. Eine rekursive Vorschrift beschreibt ein Glied durch seine Vorgänger plus Startwerte (Fibonacci: $f_1=1$, $f_2=1$, $f_n=f_{n-1}+f_{n-2}$ für $n>2$), eine explizite Vorschrift liefert $a_n$ direkt aus $n$ (z. B. $a_n=n^2$).",
    "kriterien": [
      "Folge als Funktion $\\mathbb{N} \\to X$ mit Eindeutigkeit der Zuordnung benannt (Fachsprache)",
  ...
  },
  {
    ...
    "musterantwort": "Mit $\\tilde{f}_1=4$, $\\tilde{f}_2=7$ und $\\tilde{f}_n=\\tilde{f}_{n-2}+\\tilde{f}_{n-1}$ ergibt sich 4, 7, 11, 18, 29, 47, 76, …; die Summe der ersten sechs Glieder ist $4+7+11+18+29+47=116$. Schreibt man die Glieder als Vielfache der Startzahlen, erkennt man $\\tilde{f}_n=f_{n-2} \\cdot s_1+f_{n-1} \\cdot s_2$ mit $s_1=4$, $s_2=7$, also z. B. $\\tilde{f}_6=f_4 \\cdot 4+f_5 \\cdot 7=3 \\cdot 4+5 \\cdot 7=12+35=47$. Die Koeffizienten sind gerade die gewöhnlichen Fibonacci-Zahlen.",
    "kriterien": [
      "korrekte Fortsetzung der Folge und richtige Summe (116)",
      "Darstellung $\\tilde{f}_n=f_{n-2} \\cdot s_1+f_{n-1} \\cdot s_2$ genannt und angewandt",
  ...
  },
  {
    ...
    "schwerpunkt": "zahlfolgen",
    "stufe": "beweis",
    "frage": "Beweisen Sie die Gaußsche Summenformel $1+2+3+\\dots+n = \\frac{n \\cdot (n+1)}{2}$ mit vollständiger Induktion. Beschreiben Sie Induktionsanfang und Induktionsschritt vollständig.",
    "musterantwort": "Behauptung $A(n)$: $\\sum_{k=1}^{n} k = \\frac{n(n+1)}{2}$. Induktionsanfang $n=1$: linke Seite 1, rechte Seite $\\frac{1 \\cdot 2}{2}=1$, also gilt $A(1)$. Induktionsschritt $n \\to n+1$: Es gelte $A(n)$ (Induktionsvoraussetzung). Dann ist $\\sum_{k=1}^{n+1} k = \\left(\\sum_{k=1}^{n} k\\right) + (n+1) = \\frac{n(n+1)}{2} + (n+1) = (n+1) \\cdot \\left(\\frac{n}{2}+1\\right) = \\frac{(n+1)(n+2)}{2}$, was genau $A(n+1)$ ist. Nach dem Induktionsprinzip gilt $A(n)$ für alle $n \\in \\mathbb{N}$. (Alternativ: Beweisidee von Gauß durch paarweise Summierung erster und letzter Term, $\\frac{n}{2}$ Paare zu je $n+1$.)",
    "kriterien": [
      "Induktionsanfang korrekt geprüft ($n=1$)",
      "Induktionsvoraussetzung explizit benutzt und Schritt algebraisch korrekt zu $\\frac{(n+1)(n+2)}{2}$ umgeformt",
      "Schlussfolgerung auf alle $n \\in \\mathbb{N}$ (Induktionsprinzip) formuliert"
    ],
    "quelle": "MAL1-1 – Zahlfolgen, figurierte Zahlen & Summenformeln",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Zahlfolgen, figurierte Zahlen & Summenformeln"
  },
  {
    ...
    "musterantwort": "Die $n$-te Dreieckszahl $D_n$ zählt die zu einem Dreieck gelegten Punkte und ist genau $D_n=1+2+\\dots+n=\\frac{n(n+1)}{2}$ – die figurierte Deutung der Gaußschen Summe. Legt man zwei aufeinanderfolgende Dreieckszahlen zusammen, entsteht ein Quadrat: $D_{n-1}+D_n=n^2$. Daraus folgt geometrisch (L-förmige Winkelhaken/Gnomone), dass die Summe der ersten $n$ ungeraden Zahlen $1+3+5+\\dots+(2n-1)=n^2$ ist; jede ungerade Zahl ergänzt das $(n-1)^2$-Quadrat zum $n^2$-Quadrat. So verbinden figurierte Zahlen anschauliche Geometrie mit algebraischen Summenformeln und liefern zugleich Beweisideen.",
    "kriterien": [
      "Dreieckszahl $D_n=\\frac{n(n+1)}{2}$ als figurierte Gauß-Summe erkannt",
      "Zusammenhang $D_{n-1}+D_n=n^2$ bzw. Summe ungerader Zahlen $=n^2$ begründet",
      "Verknüpfung geometrische Anschauung $\\leftrightarrow$ algebraische Formel dargestellt"
    ],
    "quelle": "MAL1-1 – Zahlfolgen, figurierte Zahlen & Summenformeln",
    "gruppe": "Arithmetik",
    "schwerpunkt_name": "Zahlfolgen, figurierte Zahlen & Summenformeln"
  ...
  },
  {
    ...
    "musterantwort": "Grundaufgabe: Gegeben sind Drehzentrum $Z$ und Drehwinkel $\\alpha$; man dreht z.B. ein Dreieck $ABC$ um $\\alpha$ um $Z$, wobei jeder Punkt seinen Abstand zu $Z$ behält (aus $PZ=3{,}36$ folgt $P'Z=3{,}36$). Umkehraufgabe: Gegeben sind zwei kongruente Figuren, gesucht sind $Z$ und $\\alpha$. Man verbindet je einen Punkt mit seinem Bildpunkt ($P$ mit $P'$, $Q$ mit $Q'$) und errichtet die Mittelsenkrechten dieser Strecken; ihr Schnittpunkt ist $Z$, weil $Z$ von Punkt und Bildpunkt gleich weit entfernt ist. Der Drehwinkel ist dann $\\alpha = \\angle PZP'$.",
    "kriterien": [
      "Unterscheidet Grundaufgabe ($Z$, $\\alpha$ gegeben) und Umkehraufgabe (Figuren gegeben) klar",
      "Konstruktion über Mittelsenkrechten der Verbindungsstrecken zum Schnittpunkt $Z$",
      "Nutzt Abstandstreue zu $Z$ und bestimmt $\\alpha$ als Winkel $\\angle PZP'$"
    ],
    "quelle": "MAL1-1 – Kongruenz-/Ähnlichkeitsabbildungen & Symmetrie",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Kongruenz-/Ähnlichkeitsabbildungen & Symmetrie"
  },
  {
    ...
    "schwerpunkt": "abbildungen",
    "stufe": "beweis",
    "frage": "Begründen Sie den Satz über die Verkettung zweier Geradenspiegelungen $S_h \\circ S_g$: Warum ergibt sich bei sich in $S$ schneidenden Geraden mit Schnittwinkel $\\alpha$ eine Drehung um $S$ mit dem Winkel $2\\alpha$ (und nicht $\\alpha$)?",
    "musterantwort": "Man führt zuerst $S_g$ und dann $S_h$ aus (Konvention von innen nach außen). Der Schnittpunkt $S$ liegt auf beiden Achsen und ist damit Fixpunkt beider Spiegelungen, also Fixpunkt der Verkettung – daher kommt nur eine Drehung um $S$ in Frage. Betrachtet man einen Strahl von $S$ unter dem Winkel $\\varphi$ zu $g$: die Spiegelung an $g$ bringt ihn nach $-\\varphi$ (Winkel $\\varphi$ auf der anderen Seite von $g$), die anschließende Spiegelung an $h$ (die zu $g$ den Winkel $\\alpha$ einschließt) fügt insgesamt den doppelten Achsenwinkel hinzu. Jede Einzelspiegelung trägt den Winkel zur jeweiligen Achse doppelt bei, sodass sich die Drehung um $2\\alpha$ ergibt (tragende Beweisidee: gemeinsamer Fixpunkt $S \\Rightarrow$ Drehung, addierte Achsenwinkel $\\Rightarrow 2\\alpha$). Die Fälle $g=h \\to$ Identität und $g \\parallel h \\to$ Verschiebung (um $2 \\cdot$ Achsenabstand) fügen sich analog ein.",
    "kriterien": [
      "Erkennt $S$ als gemeinsamen Fixpunkt beider Achsen $\\Rightarrow$ Ergebnis muss Drehung um $S$ sein",
      "Trägt die Winkelverdopplung $2\\alpha$ schlüssig (jede Spiegelung spiegelt an ihrer Achse, Achsenwinkel addieren sich)",
      "Ordnet die Sonderfälle $g=h$ (id) und $g \\parallel h$ (Verschiebung um doppelten Abstand) ein"
    ],
    "quelle": "MAL1-1 – Kongruenz-/Ähnlichkeitsabbildungen & Symmetrie",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Kongruenz-/Ähnlichkeitsabbildungen & Symmetrie"
  ...
  },
  {
    ...
    "musterantwort": "Der Flächeninhalt ordnet jeder Figur eine nichtnegative Maßzahl zu. Ein Flächenmaß $A$ erfüllt: $A(\\varnothing)=0$ und $A(F) \\ge 0$ (Nichtnegativität); Additivität, d.h. wenn $F$ und $G$ keine inneren Punkte gemeinsam haben (ihre Ränder dürfen sich berühren; formal $F^\\circ \\cap G^\\circ = \\varnothing$), dann $A(F \\cup G)=A(F)+A(G)$; Kongruenzinvarianz (kongruente Figuren haben denselben Flächeninhalt); und die Normierung $A(E)=1$ für das Einheitsquadrat $E$. Das Einheitsquadrat legt als vereinbarte Maßeinheit $e^2$ fest, gegen die alle anderen Flächen gemessen werden.",
    "kriterien": [
      ...
      "Erklärt das Einheitsquadrat als Normierungs-/Maßeinheit $e^2$"
    ],
  ...
  },
  {
    ...
    "schwerpunkt": "flaechen",
    "stufe": "verfahren",
    "frage": "Erklären Sie an einem Beispiel, wie man den Flächeninhalt eines Rechtecks durch Auslegen mit Einheitsquadraten bestimmt, und leiten Sie so die Formel $A = a \\cdot b$ her. Wo stößt dieses Verfahren an seine Grenzen?",
    "musterantwort": "Man legt das Rechteck lückenlos und überlappungsfrei mit Einheitsquadraten der Seitenlänge $e$ aus. Ein Rechteck mit den Seiten $a = n \\cdot e$ und $b = m \\cdot e$ enthält $n$ Quadrate pro Reihe und $m$ Reihen, also $n \\cdot m$ Einheitsquadrate; wegen $A(E)=e^2$ folgt $A(R) = (n \\cdot m) \\cdot e^2 = a \\cdot b$. Beispiel: Bei $a = 5e$ und $b = 2e$ passen $5 \\cdot 2 = 10$ Einheitsquadrate hinein, also $A(R) = 10e^2$. Das Auslegen gelingt exakt nur, wenn die Seitenlängen ganzzahlige Vielfache von $e$ (bzw. eines feineren Einheitsquadrats) sind. Bei inkommensurablen bzw. irrationalen Seitenlängen (etwa $\\pi$ m) oder krummlinig begrenzten Figuren versagt das reine Auslegen; man benötigt eine Verfeinerung und letztlich einen Grenzwertprozess.",
    "kriterien": [
      "Führt das Auslegen konkret durch und begründet $A = n \\cdot m \\cdot e^2 = a \\cdot b$",
      "Rechnet ein konkretes Zahlenbeispiel korrekt",
      "Benennt die Grenze (inkommensurable/irrationale Seiten, krummlinige Figuren $\\to$ Verfeinerung/Grenzwert)"
    ],
    "quelle": "MAL1-1 – Ebene Figuren, Flächeninhalte & Zerlegungsgleichheit",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Ebene Figuren, Flächeninhalte & Zerlegungsgleichheit"
  ...
  },
  {
    ...
    "kriterien": [
      ...
      "Stellt den Zusammenhang zum Flächeninhalt her (beide $\\Rightarrow$ flächengleich)",
  ...
  },
  {
    ...
    "musterantwort": "Man stellt den Satz des Pythagoras nach der gesuchten Kathete um. Mit $a = 3$ cm (Kathete) und $c = 8$ cm (Hypotenuse) gilt $a^2 + b^2 = c^2$, also $b^2 = c^2 - a^2 = 8^2 - 3^2 = 64 - 9 = 55$. Damit ist $b = \\sqrt{55} \\approx 7{,}42$ cm. Wichtig ist, zuerst zu prüfen, welche Seite die Hypotenuse ist (die längste, dem rechten Winkel gegenüber), damit man subtrahiert statt addiert.",
    "kriterien": [
      "Richtige Umstellung $b^2 = c^2 - a^2$ (Subtraktion, nicht Addition)",
      "Korrekte Rechnung mit Ergebnis $\\sqrt{55} \\approx 7{,}42$ cm",
  ...
  },
  {
    ...
    "musterantwort": "In ein großes Quadrat der Seitenlänge $(a+b)$ legt man vier zum Ausgangsdreieck kongruente rechtwinklige Dreiecke (Katheten $a$, $b$, Hypotenuse $c$; kongruent nach SSS bzw. SWS). Ihre Hypotenusen bilden die Ränder der inneren Figur. Deren Seiten haben Länge $c$; die Innenwinkel sind rechte Winkel, weil an jeder Ecke die beiden spitzen Dreieckswinkel $\\alpha$ und $\\beta$ mit $\\alpha+\\beta = 90^\\circ$ zusammen mit dem gesuchten Winkel eine gestreckte Situation ergeben, sodass der Innenwinkel $180^\\circ - (\\alpha+\\beta) = 90^\\circ$ beträgt. Die innere Figur ist also ein Quadrat mit Fläche $c^2$. Flächenbilanz: $(a+b)^2 = 4 \\cdot (\\tfrac{1}{2}ab) + c^2$, d. h. $a^2 + 2ab + b^2 = 2ab + c^2$. Kürzen von $2ab$ liefert $a^2 + b^2 = c^2$.",
    "kriterien": [
      "Kongruenz der vier Dreiecke begründet (SSS/SWS)",
      "Nachweis, dass die innere Figur ein Quadrat ist ($\\alpha+\\beta=90^\\circ \\Rightarrow$ rechter Innenwinkel), Seite $c$",
      "Vollständige Flächenbilanz $(a+b)^2 = 4 \\cdot \\tfrac{1}{2}ab + c^2$ mit korrektem Kürzen zu $a^2+b^2=c^2$"
    ],
    "quelle": "MAL1-1 – Satzgruppe des Pythagoras",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Satzgruppe des Pythagoras"
  },
  {
    ...
    "musterantwort": "Alle drei bilden die Satzgruppe des Pythagoras am rechtwinkligen Dreieck mit der Höhe $h_c$, die die Hypotenuse in die Abschnitte $p$ und $q$ teilt ($c = p + q$). Der Kathetensatz besagt $a^2 = p\\cdot c$ und $b^2 = q\\cdot c$; der Höhensatz $h_c^2 = p\\cdot q$. Herleitung des Höhensatzes: Nach dem Kathetensatz gilt $a^2 = p\\cdot c$ und im Teildreieck $a^2 = h_c^2 + p^2$, also $h_c^2 = a^2 - p^2 = p\\cdot c - p^2 = p\\,(c-p) = p\\cdot q$ (wegen $c-p = q$). Addiert man die beiden Kathetensatz-Gleichungen, erhält man $a^2 + b^2 = p\\cdot c + q\\cdot c = (p+q)\\cdot c = c^2$, also den Satz des Pythagoras. Die Umkehrung dreht Voraussetzung und Folgerung um: Wenn $a^2 + b^2 = c^2$ gilt, dann ist das Dreieck rechtwinklig; sie ist mit dem Satz selbst noch nicht bewiesen und braucht ein eigenes Argument, z. B. über Kongruenz: Man konstruiert ein rechtwinkliges Dreieck mit den Katheten $a$ und $b$; dessen Hypotenuse hat nach Pythagoras die Länge $\\sqrt{a^2+b^2} = c$, sodass das Ausgangsdreieck nach SSS zu diesem kongruent und damit ebenfalls bei $C$ rechtwinklig ist. Zusammen mit dem Satz liefert die Umkehrung die Genau-dann-wenn-Aussage.",
    "kriterien": [
  ...
  },
  {
    ...
    "kriterien": [
      ...
      "Nennt konkrete typische Schülerfehler (z. B. Umfang/Fläche, $a^2$ fehlgedeutet, Hypotenuse verkannt)"
    ],
    "quelle": "MAL1-1 – Satzgruppe des Pythagoras",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Satzgruppe des Pythagoras"
  },
  {
    ...
    "musterantwort": "Eine Gerade $m_{AB}$ heißt genau dann Mittelsenkrechte zur Strecke $AB$, wenn sie durch den Mittelpunkt von $AB$ verläuft und senkrecht auf $AB$ steht (beide Bedingungen zusammen). Gleichwertig ist die Ortsbeschreibung: Die Mittelsenkrechte ist der geometrische Ort aller Punkte $P$ der Ebene, die von $A$ und $B$ den gleichen Abstand haben ($|PA| = |PB|$).",
    "kriterien": [
  ...
  },
  {
    ...
    "kriterien": [
      "Gleicher Zirkelradius ($> \\tfrac{1}{2} \\cdot AB$) um $A$ und um $B$, Verbindung der beiden Schnittpunkte mit dem Lineal",
  ...
  },
  {
    ...
    "musterantwort": "Sei $M_U$ der Schnittpunkt der Mittelsenkrechten $m_{AB}$ und $m_{BC}$. Weil $M_U$ auf $m_{AB}$ liegt, gilt $|M_U A| = |M_U B|$; weil $M_U$ auch auf $m_{BC}$ liegt, gilt $|M_U B| = |M_U C|$. Durch Transitivität folgt $|M_U A| = |M_U B| = |M_U C|$. Wegen $|M_U A| = |M_U C|$ erfüllt $M_U$ die Ortseigenschaft der dritten Mittelsenkrechten $m_{CA}$ und liegt daher auch auf ihr — alle drei schneiden sich also in $M_U$. Da $M_U$ von $A$, $B$ und $C$ denselben Abstand hat, ist der Kreis um $M_U$ mit diesem Radius der Kreis durch $A$, $B$, $C$, also der Umkreis, und $M_U$ sein Mittelpunkt.",
    "kriterien": [
      "Nutzt die Ortseigenschaft (Punkt auf Mittelsenkrechter $\\Rightarrow$ gleiche Abstände) als tragendes Argument",
  ...
  },
  {
    ...
    "musterantwort": "Die Innenwinkelsumme des $n$-Ecks beträgt $(n-2) \\cdot 180^\\circ$, für $n=6$ also $(6-2) \\cdot 180^\\circ = 720^\\circ$. Da alle sechs Innenwinkel gleich groß sind, misst jeder $720^\\circ : 6 = 120^\\circ$. An einer Ecke des Parketts müssen sich die zusammenstoßenden Winkel zu genau $360^\\circ$ (Vollwinkel) ergänzen. Wegen $3 \\cdot 120^\\circ = 360^\\circ$ treffen sich genau drei reguläre Sechsecke lückenlos in einem Punkt, also parkettieren reguläre Sechsecke die Ebene.",
    "kriterien": [
      "Innenwinkel korrekt zu $120^\\circ$ berechnet (über Innenwinkelsumme oder $360^\\circ$-Bezug)",
      "Kriterium 'Winkel um eine Ecke ergeben $360^\\circ$' angewandt",
      "$3 \\cdot 120^\\circ = 360^\\circ$ als Begründung des Parkettierens genannt"
    ],
    "quelle": "MAL1-1 – Reguläre Polygone, Bandornamente & Parkette",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Reguläre Polygone, Bandornamente & Parkette"
  },
  {
    ...
    "musterantwort": "Der Innenwinkel eines regelmäßigen $n$-Ecks ist $\\frac{(n-2) \\cdot 180^\\circ}{n}$. Damit sich in einer Ecke mehrere gleiche Polygone lückenlos zum Vollwinkel schließen, muss dieser Innenwinkel ein Teiler von $360^\\circ$ sein, d.h. $360^\\circ$ geteilt durch den Innenwinkel muss eine natürliche Zahl $\\ge 3$ ergeben. Prüft man die Innenwinkel: Dreieck $60^\\circ$ ($360^\\circ : 60^\\circ = 6$), Quadrat $90^\\circ$ ($360^\\circ : 90^\\circ = 4$), Sechseck $120^\\circ$ ($360^\\circ : 120^\\circ = 3$) — alle gehen ganzzahlig auf. Das reguläre Fünfeck hat $108^\\circ$ ($360^\\circ : 108^\\circ = 3{,}33\\dots$ nicht ganzzahlig), und ab $n=7$ liegt der Innenwinkel echt zwischen $120^\\circ$ und $180^\\circ$, sodass $360^\\circ$ nur zwischen 2 und 3 Polygone fasst — nie ganzzahlig. Also parkettieren allein 3-, 4- und 6-Ecke.",
    "kriterien": [
      "Bedingung 'Innenwinkel ist Teiler von $360^\\circ$' als tragendes Argument",
      "Innenwinkelformel $\\frac{(n-2) \\cdot 180^\\circ}{n}$ bzw. konkrete Werte $60^\\circ$/$90^\\circ$/$120^\\circ$ benutzt",
      "Ausschluss des Fünfecks ($108^\\circ$) und größerer $n$-Ecke schlüssig begründet"
    ],
    "quelle": "MAL1-1 – Reguläre Polygone, Bandornamente & Parkette",
    "gruppe": "Geometrie",
    "schwerpunkt_name": "Reguläre Polygone, Bandornamente & Parkette"
  },
  {
    ...
    "musterantwort": "Die Bedingung 'Innenwinkel ist Teiler von $360^\\circ$' gilt nur für reguläre Polygone, bei denen alle Ecken gleich sind. Beim regulären Fünfeck scheitert das an $108^\\circ$, das kein Teiler von $360^\\circ$ ist. Ein allgemeines Fünfeck ist nicht an gleiche Winkel gebunden; passende Fünfeckstypen lassen sich so kombinieren, dass sich an jeder Ecke die verschiedenen Winkel zu $360^\\circ$ ergänzen. Beim Dreieck gilt sogar universell: Die Innenwinkelsumme ist stets $180^\\circ$. Legt man zwei kongruente Exemplare punktsymmetrisch (um $180^\\circ$ gedreht) zusammen, entsteht ein Parallelogramm, und Parallelogramme parkettieren die Ebene durch Verschieben immer — deshalb geht es mit allen Dreiecken. Analog parkettiert auch jedes beliebige Viereck (Winkelsumme $360^\\circ$).",
    "kriterien": [
      "Unterschied reguläres vs. allgemeines Polygon (feste vs. mischbare Winkel) klar",
      "$108^\\circ$ als Grund für Scheitern des regulären Fünfecks",
      "Dreieck: Winkelsumme $180^\\circ$ / Zusammensetzen zum Parallelogramm als Begründung",
  ...
  },
  {
    ...
    "kriterien": [
      "Erklärt Teilmengenbeziehung: 11 Netze $\\subset$ 35 Hexominos, mit Begründung (Faltbarkeit)",
